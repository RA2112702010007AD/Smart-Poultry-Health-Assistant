...
// Generate realistic mock historic trends for the dashboard chart
const initialHistory: PredictionRecord[] = [];
const now = new Date();

// Create 12 historical coordinates (1 hour intervals)
const sensorTrendData = [
  ...
  // High ventilation failure 8 hours ago
  ...
  // Farmers turned on fans 5 hours ago
...
sensorTrendData.forEach((trend, idx) => {
  ...
  // Predict using our RF Model
  const mlResult = rfModel.predict([trend.temp, trend.humid, trend.co2, trend.amm]);
  
  // Generate a mock static scientific advice corresponding to their risks
...
// Rule-based high-fidelity guidance serving as fail-safe backup/calibration
function getFallbackRuleRecommendations(temp: number, humid: number, co2: number, ammonia: number, level: RiskLevel): GeminiRecommendations {
  if (level === 'Healthy') {
    return {
      ...
      causes: [
        "Optimal ventilation cycling",
        "Dry litter maintenance",
        "Adequate bird stocking density"
      ],
  ...
  if (level === 'Low Risk') {
    return {
      ...
      causes: [
        "Insufficient air intake cycles",
        "Slightly damp litter crust under drinker lines",
        "Fluctuating seasonal outdoor humidity"
      ],
      immediatePreventiveMeasures: [
...
// Perform Risk Prediction and route through Gemini
app.post('/api/predict', async (req, res) => {
  try {
    ...
    // 1. Run through mathematical TypeScript Random Forest Classification model
    const mlPrediction = rfModel.predict([temp, humid, co2Val, amm]);

    // 2. Feed environmental metrics + prediction results to Google Gemini
    ...
    if (ai) {
        ...
        responseSchema: {
          ...
          properties: {
            ...
            causes: { 
              ...
              description: "List 2 to 3 practical farm causes for this state."
            },
            immediatePreventiveMeasures: { 
              type: Type.ARRAY, 
              items: { type: Type.STRING },
              description: "List 2 to 3 rapid physical actions a farmer must immediately do (e.g., raking litter, turning fans on)."
            },
            environmentalAdjustments: { 
              type: Type.ARRAY, 
              items: { type: Type.STRING },
              description: "List 2 environmental settings coordinates adjusts (ventilation speed, thermostat adjustments, opening curtains)."
            },
            feedingAndHygiene: { 
              type: Type.ARRAY, 
              items: { type: Type.STRING },
              description: "List 2 simple nutritional or sanitation adjustments (vitamins, water sanitation)."
            },
      ...
      for (const modelName of modelsToTry) {
        ...
        for (let attempt = 1; attempt <= 2; attempt++) {
          try {
            ...
            // Check for service unavailable (503) or rate limits (429) to speed-failover instantly
      ...
      // Use premium rule-based fallback when key is not loaded
      aiRecs = getFallbackRuleRecommendations(temp, humid, co2Val, amm, mlPrediction.riskLevel);
      aiRecs.isFallback = true;
      aiRecs.fallbackReason = 'No API key configured';
    }

    // Include the feature importance dynamically in prediction object
...
// Endpoint to delete a history item if requested
